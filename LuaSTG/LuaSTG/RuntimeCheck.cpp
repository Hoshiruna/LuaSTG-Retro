...
#pragma warning(pop)
    ...
    inline std::string_view make_download_guide_text(bool zh_cn)
    {
        if(zh_cn) {
            ...
            return "Several updates were not installed on the system. Download from Microsoft official website?\n"
                   "\n"
                   "Notice:\n"
    ...
    static bool CheckSystem()
    {
...

        {
            ...
            if(is_any_satisfied) {
                reset_counter();
                if(windows_7)
                    message_buffer << indent_string << next_counter_string() << windows_7_name << "\n";
                if(service_pack_1)
                    message_buffer << indent_string << next_counter_string() << service_pack_1_name << "\n";
                if(platform_update)
                    message_buffer << indent_string << next_counter_string() << platform_update_name << "\n";
                if(secure_library_loading)
                    message_buffer << indent_string << next_counter_string() << secure_library_loading_name << "\n";
            } else {
                message_buffer << "    N/A\n"; // 全都不满足，所以满足列表为空
            }
