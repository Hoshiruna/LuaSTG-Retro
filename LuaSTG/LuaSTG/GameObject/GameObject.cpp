...
#define FOR_EACH_CALLBACKS(S)                                         \
    if(isDirectCallbacks(this)) {                                     \
        reinterpret_cast<IGameObjectCallbacks*>(callbacks)->S return; \
    }                                                                 \
    for(uint32_t i = 0; i < callbacks_count; i++) { callbacks[i]->S }

    void GameObject::dispatchOnQueueToDestroy(std::string_view const reason)
    {
        FOR_EACH_CALLBACKS(onQueueToDestroy(this, reason);)
    }
    void GameObject::dispatchOnUpdate()
    {
        FOR_EACH_CALLBACKS(onUpdate(this);)
    }
    void GameObject::dispatchOnLateUpdate()
    {
        FOR_EACH_CALLBACKS(onLateUpdate(this);)
    }
    void GameObject::dispatchOnRender()
    {
        FOR_EACH_CALLBACKS(onRender(this);)
    }
    void GameObject::dispatchOnTrigger(GameObject* const other)
    {
        FOR_EACH_CALLBACKS(onTrigger(this, other);)
    }
