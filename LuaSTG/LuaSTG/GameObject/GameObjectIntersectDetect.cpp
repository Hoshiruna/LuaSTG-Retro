...
namespace
{
    ...
    constexpr bool isAxisAlignedBoundingBoxIntersect(
        ...
        const T b2) noexcept
    {
        return (x1 + a1) >= (x2 - a2) && (x1 - a1) <= (x2 + a2) && (y1 + b1) >= (y2 - b2) && (y1 - b1) <= (y2 + b2);
    }

    bool isAxisAlignedBoundingBoxIntersect(
        luastg::GameObject const* const p1,
        luastg::GameObject const* const p2) noexcept
    {
        return isAxisAlignedBoundingBoxIntersect(
            p1->x, p1->y, p1->col_r, p1->col_r, p2->x, p2->y, p2->col_r, p2->col_r);
    }
    ...
    {
        return p->rect ? xmath::collision::ColliderType::OBB : p->a == p->b ? xmath::collision::ColliderType::Circle :
...
namespace luastg
{
    bool GameObject::isIntersect(GameObject const* const p1, GameObject const* const p2) noexcept
    {
        ...
        if(!xmath::collision::check(
               xy1, r1, r1, rot1, xmath::collision::ColliderType::Circle, xy2, r2, r2, rot2, xmath::collision::ColliderType::Circle)) {
            return false;
        }

        // 精确碰撞检测
        return xmath::collision::check(
            xy1, a1, b1, rot1, getColliderType(p1), xy2, a2, b2, rot2, getColliderType(p2));
    }
}
