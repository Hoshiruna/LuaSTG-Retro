...
namespace luastg
{
    ...
    public:
        ...
        {
            return x >= m_BoundLeft && x <= m_BoundRight && y >= m_BoundBottom && y <= m_BoundTop;
        }
    ...
    public:
        ...
        {
            if(CheckWorld(a, m_Worlds[0]) && CheckWorld(b, m_Worlds[0]))
                return true;
            if(CheckWorld(a, m_Worlds[1]) && CheckWorld(b, m_Worlds[1]))
                return true;
            if(CheckWorld(a, m_Worlds[2]) && CheckWorld(b, m_Worlds[2]))
                return true;
            if(CheckWorld(a, m_Worlds[3]) && CheckWorld(b, m_Worlds[3]))
                return true;
            return false;
        }
#endif // USING_MULTI_GAME_WORLD
