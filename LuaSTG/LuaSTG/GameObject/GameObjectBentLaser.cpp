...
GameObjectBentLaser*
GameObjectBentLaser::AllocInstance()
{
#ifndef NDEBUG
...
void
GameObjectBentLaser::FreeInstance(GameObjectBentLaser* p)
{
...
void
GameObjectBentLaser::_UpdateNodeVertexExtend(size_t i) noexcept
{
    // 当前节点，先去掉大转角标记
...
void
GameObjectBentLaser::_UpdateAllNode() noexcept
{
    // 无论如何都重置长度
    m_fLength = 0.0f;
...
void
GameObjectBentLaser::_PopHead() noexcept
{
    if(m_Queue.size() > 1) {
...
int
GameObjectBentLaser::GetSize() noexcept
{
    return (int)m_Queue.size();
}

GameObjectBentLaser::LaserNode*
GameObjectBentLaser::GetNode(size_t i) noexcept
{
    if(m_Queue.size() > 0u) {
...
void
GameObjectBentLaser::GetEnvelope(float& height, float& base, float& rate, float& power) noexcept
{
...
void
GameObjectBentLaser::SetEnvelope(float height, float base, float rate, float power) noexcept
{
...
bool
GameObjectBentLaser::Update(size_t id, int length, float width, bool active) noexcept
{
...
bool
GameObjectBentLaser::Update(float x, float y, float rot, int length, float width, bool active) noexcept
{
...
void
GameObjectBentLaser::SetAllWidth(float width) noexcept
{
...
bool
GameObjectBentLaser::Render(const char* tex_name, BlendMode blend, core::Color4B c, float tex_left, float tex_top, float tex_width, float tex_height, float scale) noexcept
{
...
void
GameObjectBentLaser::RenderCollider(core::Color4B fillColor) noexcept
{
    // 忽略只有一个节点的情况
    if(m_Queue.size() <= 1)
        return;
...
bool
GameObjectBentLaser::CollisionCheck(float x, float y, float rot, float a, float b, bool rect) noexcept
{
    // 忽略只有一个节点的情况
    if(m_Queue.size() <= 1)
        return false;
...
bool
GameObjectBentLaser::CollisionCheckW(float x, float y, float rot, float a, float b, bool rect, float width) noexcept
{
    // 忽略只有一个节点的情况
    if(m_Queue.size() <= 1)
        return false;
...
bool
GameObjectBentLaser::BoundCheck() noexcept
{
    auto& manager = LPOOL;
...
bool
GameObjectBentLaser::UpdateByNode(size_t id, int node, int length, float width, bool active) noexcept
{
...
bool
GameObjectBentLaser::UpdatePositionByList(lua_State* L, int length, float width, int index, bool revert) noexcept // ... t(list) //lua index从1开始
{
    // ... t(list)
...
int
GameObjectBentLaser::SampleL(lua_State* L, float length) noexcept
{
    //插入一个数组
...
int
GameObjectBentLaser::SampleT(lua_State* L, float delay) noexcept
{
    //插入一个数组
    lua_newtable(L); //... t(list)
    // 忽略没有节点的情况
...
inline bool
luaL_isnumber(lua_State* L, int idx)
{
    return LUA_TNUMBER == lua_type(L, idx);
}

int
GameObjectBentLaser::api_UpdateSingleNode(lua_State* L)
{
    ...
    if(m_Queue.size() > 1) {
        if(node_index > 0)
            _UpdateNodeVertexExtend(node_index - 1);
        if(node_index < (m_Queue.size() - 1))
            _UpdateNodeVertexExtend(node_index + 1);
    }
...
int
GameObjectBentLaser::api_UpdateAllNodeByList(lua_State* L)
{
    // self, size, x[], y[], width?
    assert(lua_gettop(L) == 5);
    ...
    if(read_width) {
        ...
        for(size_t i = 0; i < node_count; i += 1) {
            ...
            if(!luaL_isnumber(L, 6))
                return luaL_error(L, "invalid number at [%d] in parameter #2", luai);
            if(!luaL_isnumber(L, 7))
                return luaL_error(L, "invalid number at [%d] in parameter #3", luai);
        ...
        for(size_t i = 0; i < node_count; i += 1) {
            ...
            if(!luaL_isnumber(L, 7))
                return luaL_error(L, "invalid number at [%d] in parameter #3", luai);
            if(!luaL_isnumber(L, 8))
                return luaL_error(L, "invalid number at [%d] in parameter #4", luai);
            node.pos.x = (float)lua_tonumber(L, 6);
            node.pos.y = (float)lua_tonumber(L, 7);
