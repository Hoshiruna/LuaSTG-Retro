...
private:
    ...
    static int integer(lua_State* L)
    {
        ...
        int const argc = lua_gettop(L);
        if(argc == 1) {
            lua_pushinteger(L, self->int_gn(self->rng, std::uniform_int_distribution<lua_Integer>::param_type(0, std::numeric_limits<lua_Integer>::max())));
            return 1;
        } else if(argc == 2) {
    ...
    static int number(lua_State* L)
    {
        ...
        int const argc = lua_gettop(L);
        if(argc == 1) {
            lua_pushnumber(L, self->num_gn(self->rng, std::uniform_real_distribution<lua_Number>::param_type(0.0, std::nextafter(1.0, std::numeric_limits<lua_Number>::max()))));
            return 1;
        } else if(argc == 2) {
            lua_Number b = luaL_checknumber(L, 2);
            if(b < 0.0)
                b = -b;
            lua_pushnumber(L, self->num_gn(self->rng, std::uniform_real_distribution<lua_Number>::param_type(0.0, std::nextafter(b, std::numeric_limits<lua_Number>::max()))));
            return 1;
        } else if(argc == 3) {
            ...
            if(a > b)
                std::swap(a, b);
            lua_pushnumber(L, self->num_gn(self->rng, std::uniform_real_distribution<lua_Number>::param_type(a, std::nextafter(b, std::numeric_limits<lua_Number>::max()))));
            return 1;
        } else {
            return luaL_error(L, "invalid parameter");
        }
...
namespace random
{
    ...
    public:
        ...
        pcg32_oneseq_ex(Args&&... args)
            : pcg32_oneseq(std::forward<Args>(args)...) {}
    ...
    public:
        template<typename... Args>
        pcg32_fast_ex(Args&&... args)
            : pcg32_fast(std::forward<Args>(args)...) {}
    ...
    public:
        template<typename... Args>
        pcg64_oneseq_ex(Args&&... args)
            : pcg64_oneseq(std::forward<Args>(args)...) {}
    ...
    public:
        template<typename... Args>
        pcg64_fast_ex(Args&&... args)
            : pcg64_fast(std::forward<Args>(args)...) {}
