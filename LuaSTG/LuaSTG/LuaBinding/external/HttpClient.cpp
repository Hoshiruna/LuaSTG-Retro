...
static void
setHeaders(HINTERNET const handle, std::unordered_map<std::string, std::string> const& headers)
{
...
trim(std::string_view const& input)
{
...
static void
parseResponseHeaders(std::string const& response_headers, std::unordered_map<std::string, std::string>& headers)
{
        ...
        static int execute(lua_State* L)
        {
            ...
            try {
                ...
                br = WinHttpSendRequest(
                    ...
                    0,
                    // ReSharper disable once CppZeroConstantCanBeReplacedWithNullptr
                    body_size > 0 ? self->body.data() : WINHTTP_NO_REQUEST_DATA,
                    body_size,
                    body_size,
                    0);
