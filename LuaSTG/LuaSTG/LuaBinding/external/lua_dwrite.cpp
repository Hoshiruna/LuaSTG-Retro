...
namespace DirectWrite
{
    ...
    {
        std::function<void()> f;
        ScopeFunction(std::function<void()> fv)
            : f(fv) {}
        ~ScopeFunction() { f(); }
    };
    ...
    void printFontCollectionInfo(IDWriteFontCollection* dwrite_font_collection, std::stringstream& string_buffer)
    {
        for(UINT32 ff_idx = 0; ff_idx < dwrite_font_collection->GetFontFamilyCount(); ff_idx += 1) {
            ...
            for(UINT32 name_idx = 0; name_idx < dwrite_font_family_names->GetCount(); name_idx += 1) {
                ...
                if(locale_name.back() == L'\0')
                    locale_name.pop_back();
            ...
            for(UINT32 font_idx = 0; font_idx < dwrite_font_family->GetFontCount(); font_idx += 1) {
                ...
                if(font_sim == DWRITE_FONT_SIMULATIONS_NONE)
                    string_buffer << "None";
                if(font_sim & DWRITE_FONT_SIMULATIONS_BOLD)
                    string_buffer << "Algorithmic Emboldening";
                if((font_sim & DWRITE_FONT_SIMULATIONS_BOLD) && (font_sim & DWRITE_FONT_SIMULATIONS_OBLIQUE))
                    string_buffer << ", ";
                if(font_sim & DWRITE_FONT_SIMULATIONS_OBLIQUE)
                    string_buffer << "Algorithmic Italicization";
                string_buffer << "\n";
    ...
    public:
        HRESULT WINAPI ReadFileFragment(void const** fragmentStart, UINT64 fileOffset, UINT64 fragmentSize, void** fragmentContext) noexcept
        {
            ...
            if((fileOffset + fragmentSize) > m_data->size())
                return E_INVALIDARG;
    ...
    static struct ModuleLoader
    {
        HMODULE dll_d2d1;
        HMODULE dll_dwrite;
        HRESULT(WINAPI* api_D2D1CreateFactory)
        (D2D1_FACTORY_TYPE, REFIID, CONST D2D1_FACTORY_OPTIONS*, void**);
        HRESULT(WINAPI* api_DWriteCreateFactory)
        (DWRITE_FACTORY_TYPE, REFIID, IUnknown**);
        void(WINAPI* api_D2D1MakeRotateMatrix)(FLOAT, D2D1_POINT_2F, D2D1_MATRIX_3X2_F*);

        ModuleLoader()
            : dll_d2d1(NULL), dll_dwrite(NULL), api_D2D1CreateFactory(NULL), api_DWriteCreateFactory(NULL), api_D2D1MakeRotateMatrix(NULL)
        {
    ...
    public:
        DWriteTextRendererImplement(
            ...
            FLOAT width)
            : d2d1_factory(factory), d2d1_rt(target), dwrite_text_layout(layout), d2d1_brush_outline(outline), d2d1_brush_fill(fill), d2d1_stroke_style(stroke_style), outline_width(width) {}
        ~DWriteTextRendererImplement() {}
    };
    ...
    struct TextRenderer
    {
        ...
        TextRenderer()
            : font_color(core::Color4B(255, 255, 255, 255)), outline_width(0.0f), outline_color(core::Color4B(0, 0, 0, 255)), shadow_radius(0.0f), shadow_color(core::Color4B(0, 0, 0, 255))
        {
        }
        ~TextRenderer() {}
    ...
    static int api_CreateTextFormat(lua_State* L)
    {
        ...
        if(lua_isuserdata(L, 2))
            font_collection = FontCollection::Cast(L, 2);
    ...
    static int api_CreateTextureFromTextLayout(lua_State* L)
    {
        ...
        if(FAILED(hr))
            return luaL_error(L, "read rasterize result failed");
        UINT buffer_size = 0;
    ...
    static void register_C_enum(lua_State* L)
    {
        ...
        CEnum<DWRITE_LINE_SPACING_METHOD, 2> const C_enum_LineSpacingMethod = { "LineSpacingMethod", {
                                                                                                         "Default", DWRITE_LINE_SPACING_METHOD_DEFAULT, "Uniform", DWRITE_LINE_SPACING_METHOD_UNIFORM,
        ...
        CEnum<DWRITE_READING_DIRECTION, 2> const C_enum_ReadingDirection = { "ReadingDirection", {
                                                                                                     "LeftToRight", DWRITE_READING_DIRECTION_LEFT_TO_RIGHT, "RightToLeft", DWRITE_READING_DIRECTION_RIGHT_TO_LEFT,
        ...
        CEnum<DWRITE_WORD_WRAPPING, 2> const C_enum_WordWrapping = { "WordWrapping", {
                                                                                         "Wrap", DWRITE_WORD_WRAPPING_WRAP, "NoWrap", DWRITE_WORD_WRAPPING_NO_WRAP,
