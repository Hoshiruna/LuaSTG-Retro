...
namespace luastg
{
    inline core::Graphics::IRenderer* LR2D()
    {
        return LAPP.GetAppModel()->getRenderer();
    }
    inline ResourceMgr& LRESMGR()
    {
...
#define validate_render_scope() \
    if(!LR2D()->isBatchScope()) \
        return luaL_error(L, "invalid render operation");
    ...
    static int lib_drawSprite(lua_State* L)
    {
        ...
        RenderError re = api_drawSprite(
            ...
            (float)luaL_optnumber(L, 6, hscale) * LRESMGR().GetGlobalImageScaleFactor(),
    ...
    static int lib_drawSpriteSequence(lua_State* L)
    {
        ...
        RenderError re = api_drawSpriteSequence(
            ...
            hscale * LRESMGR().GetGlobalImageScaleFactor(),
            (float)luaL_optnumber(L, 7, hscale) * LRESMGR().GetGlobalImageScaleFactor(),
    ...
    static int compat_PushRenderTarget(lua_State* L) noexcept
    {
        ...
        if(lua_isstring(L, 1)) {
            ...
            if(!LAPP.GetRenderTargetManager()->PushRenderTarget(texture.get())) {
                return luaL_error(L, ds ? "push RenderTarget and DepthStencilBuffer failed." : "push RenderTarget failed.");
            }
        }
            ...
            if(lua_istable(L, 4)) {
                ...
                while(0 != lua_next(L, 4)) {
                    ...
                    if(lua_isnumber(L, -1)) {
                        ...
                        p_effect->setFloat2(key, core::Vector2F(static_cast<float>(p_value->data.x), static_cast<float>(p_value->data.y)));
                    } else if(binding::Vector3::is(L, -1)) {
                        auto const p_value = binding::Vector3::as(L, -1);
                        p_effect->setFloat3(key, core::Vector3F(static_cast<float>(p_value->data.x), static_cast<float>(p_value->data.y), static_cast<float>(p_value->data.z)));
                    } else if(binding::Vector4::is(L, -1)) {
                        auto const p_value = binding::Vector4::as(L, -1);
                        p_effect->setFloat4(key, core::Vector4F(static_cast<float>(p_value->data.x), static_cast<float>(p_value->data.y), static_cast<float>(p_value->data.z), static_cast<float>(p_value->data.w)));
                    } else if(lua_isuserdata(L, -1)) {
                        auto const color = *binding::Color::Cast(L, -1);
                        p_effect->setFloat4(key, core::Vector4F(static_cast<float>(color.r) / 255.0f, static_cast<float>(color.g) / 255.0f, static_cast<float>(color.b) / 255.0f, static_cast<float>(color.a) / 255.0f));
                    } else {
...
void
luastg::binding::Renderer::Register(lua_State* L) noexcept
{
