...
#define GETUDATA(p, i) core::Color4B*(p) = Cast(L, i);
            ...
            static int Meta_Add(lua_State* L) noexcept
            {
                if(lua_isnumber(L, 1)) {
                    ...
                    GETUDATA(p, 2);
                    Color::CreateAndPush(L, core::Color4B((uint8_t)std::clamp(v + (lua_Number)p->r, 0.0, 255.0), (uint8_t)std::clamp(v + (lua_Number)p->g, 0.0, 255.0), (uint8_t)std::clamp(v + (lua_Number)p->b, 0.0, 255.0), (uint8_t)std::clamp(v + (lua_Number)p->a, 0.0, 255.0)));
                } else if(lua_isnumber(L, 2)) {
                    lua_Number const v = luaL_checknumber(L, 2);
                    GETUDATA(p, 1);
                    Color::CreateAndPush(L, core::Color4B((uint8_t)std::clamp((lua_Number)p->r + v, 0.0, 255.0), (uint8_t)std::clamp((lua_Number)p->g + v, 0.0, 255.0), (uint8_t)std::clamp((lua_Number)p->b + v, 0.0, 255.0), (uint8_t)std::clamp((lua_Number)p->a + v, 0.0, 255.0)));
                } else {
                    GETUDATA(pA, 1);
                    GETUDATA(pB, 2);
                    Color::CreateAndPush(L, core::Color4B((uint8_t)std::clamp((lua_Number)pA->r + (lua_Number)pB->r, 0.0, 255.0), (uint8_t)std::clamp((lua_Number)pA->g + (lua_Number)pB->g, 0.0, 255.0), (uint8_t)std::clamp((lua_Number)pA->b + (lua_Number)pB->b, 0.0, 255.0), (uint8_t)std::clamp((lua_Number)pA->a + (lua_Number)pB->a, 0.0, 255.0)));
                }
                return 1;
            }
            ...
            static int Meta_Mul(lua_State* L) noexcept
            {
                if(lua_isnumber(L, 1)) {
                    ...
                    GETUDATA(pA, 1);
                    GETUDATA(pB, 2);
                    Color::CreateAndPush(L, core::Color4B((uint8_t)std::clamp(255.0 * (((lua_Number)pA->r * _1_255) * ((lua_Number)pB->r * _1_255)), 0.0, 255.0), (uint8_t)std::clamp(255.0 * (((lua_Number)pA->g * _1_255) * ((lua_Number)pB->g * _1_255)), 0.0, 255.0), (uint8_t)std::clamp(255.0 * (((lua_Number)pA->b * _1_255) * ((lua_Number)pB->b * _1_255)), 0.0, 255.0), (uint8_t)std::clamp(255.0 * (((lua_Number)pA->a * _1_255) * ((lua_Number)pB->a * _1_255)), 0.0, 255.0)));
                }
                return 1;
            }
            ...
            static int Color(lua_State* L) noexcept
            {
                if(lua_gettop(L) == 1) {
                    ...
                    CreateAndPush(L, core::Color4B((uint8_t)std::clamp<lua_Integer>(luaL_checkinteger(L, 2), 0, 255), (uint8_t)std::clamp<lua_Integer>(luaL_checkinteger(L, 3), 0, 255), (uint8_t)std::clamp<lua_Integer>(luaL_checkinteger(L, 4), 0, 255), (uint8_t)std::clamp<lua_Integer>(luaL_checkinteger(L, 1), 0, 255) // 这个才是 a 通道
                                         ));
                }
                return 1;
            }
