        ...
        static int api_createSpriteSequence(lua_State* L)
        {
            ...
            if(ResourceTexture::test(L, 3)) {
                ...
                if(!getPool(L, self)->CreateAnimation(
                       sprite_sequence_name.data(), texture.get(), x, y, width, height, columns, rows, interval, a, b, rect)) {
...
luastg::ResourcePoolId
luastg::binding::checkResourcePoolId(lua_State* L, int const index)
{
    return ResourcePoolBinding::cast(L, index)->id;
}

luastg::ResourcePool*
luastg::binding::checkResourcePool(lua_State* L, int const index)
{
...
void
luastg::binding::pushResourcePool(lua_State* L, ResourcePoolId const id)
{
...
void
luastg::binding::registerResourcePoolMethods(lua_State* L, luaL_Reg const* methods)
{
...
luastg::IResourceTexture*
luastg::binding::checkResourceTexture(lua_State* L, int const index)
{
    return ResourceTexture::cast(L, index)->data;
}

luastg::IResourceSprite*
luastg::binding::checkResourceSprite(lua_State* L, int const index)
{
    return ResourceSprite::cast(L, index)->data;
}

void
luastg::binding::pushResourceTexture(lua_State* L, IResourceTexture* resource)
{
    resource->retain();
...
void
luastg::binding::pushResourceSprite(lua_State* L, IResourceSprite* resource)
{
    resource->retain();
...
void
luastg::binding::pushResourceAnimation(lua_State* L, IResourceAnimation* resource)
{
    resource->retain();
...
int
luaopen_LuaSTG_Sub(lua_State* L)
{
