...
namespace
{
    ...
    core::IAudioSystem* engine(lua_State* const vm)
    {
        ...
        if(result == nullptr)
            luaL_error(vm, "audio system is not initialized");
        return result;
    }
    ...
    uint64_t optionalFrameField(lua_State* const vm, int const table, char const* const name, uint64_t const fallback)
    {
        ...
        if(value < 0)
            luaL_error(vm, "%s cannot be negative", name);
    ...
    core::AudioBus checkBus(lua_State* const vm, int const index)
    {
        ...
        if(std::string_view(value) == "master")
            return core::AudioBus::master;
        if(std::string_view(value) == "sound_effect")
            return core::AudioBus::sound_effect;
        if(std::string_view(value) == "music")
            return core::AudioBus::music;
...
void
luastg::binding::Audio::Register(lua_State* const vm) noexcept
{
    struct Wrapper
    {
        ...
        static int PlaySound(lua_State* const vm)
        {
            ...
            if(!resource)
                return luaL_error(vm, "sound '%s' not found", name);
        ...
        static int PlayMusic(lua_State* const vm)
        {
            ...
            if(!resource)
                return luaL_error(vm, "music '%s' not found", name);
            auto parameters = readParameters(vm, 2, true);
            if(parameters.loop_range.end == 0)
                parameters.loop_range = resource->GetLoopRange();
            core::AudioVoiceHandle voice;
            auto* const audio = engine(vm);
        ...
        static int Seek(lua_State* const vm)
        {
            ...
            if(value < 0)
                return luaL_argerror(vm, 2, "frame cannot be negative");
        ...
        static int SetLoopRange(lua_State* const vm)
        {
            ...
            if(begin < 0 || end < 0)
                return luaL_error(vm, "loop frames cannot be negative");
        ...
        static int SetMaxVoices(lua_State* const vm)
        {
            ...
            if(value < 1 || value > 256)
                return luaL_argerror(vm, 1, "expected a value in [1, 256]");
