...
static int
package_loader_luastg(lua_State* L)
{
    const char* filename;
    ...
    if(!core::FileSystemManager::readFile(filename, src.put()))
        ...
        try {
            if(core::FileSystemManager::resolvePhysicalPath(filename, physical_path) && canonicalize_physical_path(physical_path, canonical_path)) {
        ...
        if(push_loaded_module_physical_paths(L, false)) { // state
            ...
            if(lua_istable(L, loaded_index)) {
                ...
                while(lua_next(L, paths_index) != 0) { // ... key path
                    ...
                    if(lua_isstring(L, key_index) && lua_isstring(L, -1) && is_current_loaded_module(L, modules_index, loaded_index, key_index)) {
