...
namespace luastg
{
    ...
    ResourceTextureImpl::ResourceTextureImpl(const char* name, core::Graphics::ITexture2D* p_texture)
        : ResourceBaseImpl(ResourceType::Texture, name), m_texture(p_texture), m_is_rendertarget(false), m_is_auto_resize(false), m_enable_depthbuffer(false)
    {
    }
    // 渲染附件容器
    ResourceTextureImpl::ResourceTextureImpl(const char* name, int w, int h, bool ds)
        : ResourceBaseImpl(ResourceType::Texture, name), m_is_rendertarget(true), m_is_auto_resize(false), m_enable_depthbuffer(ds)
    {
    ...
    ResourceTextureImpl::ResourceTextureImpl(const char* name, bool ds)
        : ResourceBaseImpl(ResourceType::Texture, name), m_is_rendertarget(true), m_is_auto_resize(true), m_enable_depthbuffer(ds)
    {
...
namespace luastg
{
    RenderTargetStackResourceTextureImpl::RenderTargetStackResourceTextureImpl(core::Graphics::IRenderTarget* const rt, core::Graphics::IDepthStencilBuffer* const ds)
        : m_rt(rt), m_ds(ds)
    {
    }
