...
namespace luastg
{
    ResourceVideoImpl::ResourceVideoImpl(char const* const name, char const* const path, bool const loop)
        : ResourceBaseImpl(ResourceType::Texture, name), m_loop(loop)
    {
    ...
    ResourceVideoImpl::ResourceVideoImpl(char const* const name, core::IVideoDecoder* const decoder, bool const loop)
        : ResourceBaseImpl(ResourceType::Texture, name), m_decoder(decoder), m_loop(loop)
    {
        if(!m_decoder) {
            throw std::runtime_error("create video decoder failed");
        }
        ...
        for(;;) {
...

            {
                ...
                m_worker_cv.wait(lock, [this] {
                    return m_worker_exit || m_worker_seek_requested || (m_worker_decode_enabled && !m_worker_eof && !m_worker_error && m_frame_queue.size() < max_queued_frames);
                });
