...
namespace luastg
{
    ...
    bool ResourceMusicImpl::IsPlaying()
    {
        return m_player->getState() == core::AudioPlayerState::playing;
    }
    bool ResourceMusicImpl::IsPaused()
    {
        return m_player->getState() == core::AudioPlayerState::paused;
    }
    bool ResourceMusicImpl::IsStopped()
    {
        return m_player->getState() == core::AudioPlayerState::stopped;
    }
    void ResourceMusicImpl::SetVolume(float const v)
    {
        m_player->setVolume(v);
    }
    float ResourceMusicImpl::GetVolume()
    {
        return m_player->getVolume();
    }
    bool ResourceMusicImpl::SetSpeed(float const speed)
    {
        return m_player->setSpeed(speed);
    }
    float ResourceMusicImpl::GetSpeed()
    {
        return m_player->getSpeed();
    }
    void ResourceMusicImpl::SetLoop(bool const v)
    {
        if(!v)
        ...
        if(m_decoder) {
            ...
            switch(range.type) {
                ...
                case MusicRoopRangeType::All:
                    ...
                    break;
                case MusicRoopRangeType::StartPointToEnd:
                    spdlog::info("[luastg] SetMusicLoopRange '{}' : StartPointToEnd ({} ({}s) -> end)", GetResName(), range.start_in_samples, double(range.start_in_samples) / sample_rate);
                    break;
                case MusicRoopRangeType::StartPointAndLength:
                    spdlog::info("[luastg] SetMusicLoopRange '{}' : StartPointAndLength ({} ({}s) + {} ({}s))", GetResName(), range.start_in_samples, double(range.start_in_samples) / sample_rate, range.length_in_samples, double(range.length_in_samples) / sample_rate);
                    break;
                case MusicRoopRangeType::LengthAndEndPoint:
                    spdlog::info("[luastg] SetMusicLoopRange '{}' : LengthAndEndPoint ({} ({}s) - {} ({}s))", GetResName(), range.length_in_samples, double(range.length_in_samples) / sample_rate, range.end_in_samples, double(range.end_in_samples) / sample_rate);
                    break;
                case MusicRoopRangeType::StartToEndPoint:
                    spdlog::info("[luastg] SetMusicLoopRange '{}' : StartToEndPoint (start -> {} ({}s))", GetResName(), range.end_in_samples, double(range.end_in_samples) / sample_rate);
                    break;
                case MusicRoopRangeType::StartPointAndEndPoint:
                    spdlog::info("[luastg] SetMusicLoopRange '{}' : StartPointAndEndPoint ({} ({}s) -> {} ({}s))", GetResName(), range.start_in_samples, double(range.start_in_samples) / sample_rate, range.end_in_samples, double(range.end_in_samples) / sample_rate);
                    break;
            }
        }
    }
    ...
    ResourceMusicImpl::ResourceMusicImpl(const char* name, core::IAudioDecoder* decoder, core::IAudioPlayer* p_player)
        : ResourceBaseImpl(ResourceType::Music, name), m_decoder(decoder), m_player(p_player)
    {
    }
>>>>>>> origin/master
}
