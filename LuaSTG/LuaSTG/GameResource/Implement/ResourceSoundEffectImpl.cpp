...
namespace luastg
{
    ...
    bool ResourceSoundEffectImpl::IsPlaying()
    {
        return m_state == core::AudioPlayerState::playing;
    }
    bool ResourceSoundEffectImpl::IsStopped()
    {
        return m_state == core::AudioPlayerState::stopped;
    }
    bool ResourceSoundEffectImpl::SetSpeed(float const speed)
    {
        return m_player->setSpeed(speed);
    }
    float ResourceSoundEffectImpl::GetSpeed()
    {
        return m_player->getSpeed();
    }

    ResourceSoundEffectImpl::ResourceSoundEffectImpl(const char* name, core::IAudioPlayer* p_player)
        : ResourceBaseImpl(ResourceType::SoundEffect, name), m_player(p_player)
    {
    }
>>>>>>> origin/master
}
