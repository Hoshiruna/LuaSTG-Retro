...
namespace luastg
{
            ...
            auto readCharData = [&path, &errorLineParser](std::string_view line) -> CharData {
                ...
                if(qp1 != std::string_view::npos && qp2 != std::string_view::npos && (qp2 - qp1) > 1) {
                    ...
                    if(sp1 != std::string_view::npos)
                        sp2 = data.find_first_of(',', sp1 + 1);
                    if(sp2 != std::string_view::npos)
                        sp3 = data.find_first_of(',', sp2 + 1);
                    if(sp3 != std::string_view::npos)
                        sp4 = data.find_first_of(',', sp3 + 1);
                    if(sp4 != std::string_view::npos)
                        sp5 = data.find_first_of(',', sp4 + 1);
                    if(sp5 != std::string_view::npos)
                        sp6 = data.find_first_of(',', sp5 + 1);
                ...
                if(sp1 != std::string_view::npos)
                    ce1 = data.substr(0, sp1);
                if(sp2 != std::string_view::npos)
                    ce2 = data.substr(sp1 + 1, sp2 - (sp1 + 1));
                if(sp3 != std::string_view::npos)
                    ce3 = data.substr(sp2 + 1, sp3 - (sp2 + 1));
                if(sp4 != std::string_view::npos)
                    ce4 = data.substr(sp3 + 1, sp4 - (sp3 + 1));
                if(sp5 != std::string_view::npos)
                    ce5 = data.substr(sp4 + 1, sp5 - (sp4 + 1));
                if(sp6 != std::string_view::npos)
                    ce6 = data.substr(sp5 + 1, sp6 - (sp5 + 1));
    ...
    public:
        f2dFont(std::string_view path, std::string_view raw_texture_path, bool mipmap)
            : m_line_height(0.0f), m_ascender(0.0f), m_descender(0.0f)
        {
            // 打开 fancy2d 字体定义文件
            core::SmartReference<core::IData> src;
            if(!core::FileSystemManager::readFile(path, src.put())) {
    ...
    ResourceFontImpl::ResourceFontImpl(const char* name, std::string_view hge_path, bool mipmap)
        : ResourceBaseImpl(ResourceType::SpriteFont, name), m_BlendMode(BlendMode::MulAlpha), m_BlendColor(core::Color4B(0xFFFFFFFFu))
    {
