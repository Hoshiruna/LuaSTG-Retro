...
namespace luastg
{
    namespace
    {
        ...
        {
            return state == AsyncResourceJobState::Done || state == AsyncResourceJobState::Failed || state == AsyncResourceJobState::Cancelled;
        }
        ...
        bool readRelatedFile(std::string_view const base_path, std::string_view const file_path, core::SmartReference<core::IData>& data, std::string& resolved_path, std::string& error)
        {
            if(file_path.empty()) {
    ...
    AsyncResourceJob::AsyncResourceJob(std::string_view const path)
        : m_kind(AsyncResourceJobKind::FileRead), m_file_path(path), m_debug_files{ std::string(path) }
    {
    }

    AsyncResourceJob::AsyncResourceJob(AsyncResourceRequest request)
        : m_kind(AsyncResourceJobKind::Resource), m_request(std::move(request))
    {
...
>>>>>>> origin/master
    ...
    bool AsyncResourceLoader::finalize(ResourceMgr& manager, AsyncResourceJob& job)
    {
        ...
        switch(request.type) {
            ...
            case AsyncResourceRequestType::Sprite:
                if(!pool->CreateSprite(request.name.c_str(), request.texture.get(), request.x, request.y, request.w, request.h, request.a, request.b, request.rect)) {
            ...
            case AsyncResourceRequestType::Animation:
                ...
                } else if(!pool->CreateAnimation(request.name.c_str(), request.texture.get(), request.x, request.y, request.w, request.h, request.columns, request.rows, request.interval, request.a, request.b, request.rect)) {
                    job.fail("failed to create animation");
                    return false;
                }
                break;
            ...
            case AsyncResourceRequestType::Particle:
                if(request.has_particle_info) {
                    if(!pool->LoadParticle(request.name.c_str(), request.particle_info, request.sprite.get(), request.a, request.b, request.rect)) {
                        job.fail("failed to load particle");
                        return false;
                    }
                } else if(!pool->LoadParticle(request.name.c_str(), job.m_particle_info, request.sprite.get(), request.a, request.b, request.rect)) {
                    job.fail("failed to load particle");
                    return false;
                }
                break;
...
>>>>>>> origin/master
            ...
            case AsyncResourceRequestType::SpriteFont:
                if(request.has_texture_path) {
                    if(!pool->LoadSpriteFont(request.name.c_str(), job.m_data.get(), request.path.c_str(), request.texture_path.c_str(), job.m_texture_data.get(), request.mipmap)) {
                        job.fail("failed to load font");
                        return false;
                    }
                } else if(!pool->LoadSpriteFont(request.name.c_str(), job.m_data.get(), request.path.c_str(), job.m_texture_data.get(), request.texture_path.c_str(), request.mipmap)) {
                    job.fail("failed to load font");
                    return false;
                }
                break;
