...
static std::string
bytes_count_to_string(unsigned long long size)
{
    int count = 0;
    char buffer[64] = {};
    if(size < 1024) // B
...
#ifdef USING_DEAR_IMGUI
static std::string
async_resource_pool_name(luastg::ResourceMgr const& manager, luastg::ResourcePoolId const id)
{
...
static char const*
async_resource_type_name(luastg::AsyncResourceJobDebugInfo const& info)
{
...
static char const*
async_resource_state_name(luastg::AsyncResourceJobState const state)
{
    switch(state) {
...
static ImVec4
async_resource_state_color(luastg::AsyncResourceJobState const state)
{
    switch(state) {
        case luastg::AsyncResourceJobState::Queued:
...
static bool
async_resource_state_finished(luastg::AsyncResourceJobState const state)
{
    return state == luastg::AsyncResourceJobState::Done || state == luastg::AsyncResourceJobState::Failed || state == luastg::AsyncResourceJobState::Cancelled;
}
#endif
            ...
            if(p_pool) {
                if(ImGui::BeginTabBar("##lstg.ResourceManager")) {
                    if(ImGui::BeginTabItem("Async Loading")) {
                        ...
                        if(ImGui::BeginTable("##lstg.AsyncResourceJobs", 6, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_Resizable)) {
