...
namespace Platform::RuntimeLoader
{
        ...
        ID2D1Device** d2dDevice)
    {
        if(api_D2D1CreateDevice) {
            D2D1_CREATION_PROPERTIES prop = {
                .threadingMode = multiThread ? D2D1_THREADING_MODE_MULTI_THREADED : D2D1_THREADING_MODE_SINGLE_THREADED,
#ifdef _DEBUG
                .debugLevel = D2D1_DEBUG_LEVEL_INFORMATION,
#else
                .debugLevel = D2D1_DEBUG_LEVEL_NONE,
#endif
                .options = multiThread ? D2D1_DEVICE_CONTEXT_OPTIONS_ENABLE_MULTITHREADED_OPTIMIZATIONS : D2D1_DEVICE_CONTEXT_OPTIONS_NONE,
            };
        ...
        ID2D1DeviceContext** d2dDeviceContext)
    {
        if(api_D2D1CreateDevice) {
            ...
            if(FAILED(hr))
                return hr;
            return (*d2dDevice)->CreateDeviceContext(multiThread ? D2D1_DEVICE_CONTEXT_OPTIONS_ENABLE_MULTITHREADED_OPTIMIZATIONS : D2D1_DEVICE_CONTEXT_OPTIONS_NONE, d2dDeviceContext);
        }
        return E_NOTIMPL;
    }
