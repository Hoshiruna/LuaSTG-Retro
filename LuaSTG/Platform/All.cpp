...
namespace Platform
{
    ...
    BOOL Direct3D11::HasDevice(UINT Flags, D3D_FEATURE_LEVEL TargetFeatureLevel)
    {
        ...
        for(UINT index = 0; SUCCEEDED(dxgi_factory->EnumAdapters1(index, &dxgi_adapter)); index += 1) {
            ...
            if(dxgi_adapter_info.Flags & DXGI_ADAPTER_FLAG_SOFTWARE)
                continue;
            if(dxgi_adapter_info.Flags & DXGI_ADAPTER_FLAG_REMOTE)
                continue;
            // test feature level
            hr = d3d11.CreateDeviceFromAdapter(dxgi_adapter.Get(), Flags, TargetFeatureLevel, NULL, NULL, NULL);
            if(SUCCEEDED(hr))
                count += 1;
        }
...
namespace Platform
{
    ...
    HRESULT DXGI::MakeSwapChainWindowAssociation(IDXGISwapChain* pSwapChain, UINT flags)
    {
        ...
        if(SUCCEEDED(hr)) {
            ...
            if(FAILED(hr))
                return hr; // the swap chain might not be HWND-based
        } else {
...
#pragma warning(pop)
    ...
    ApplicationSingleInstance::ApplicationSingleInstance(std::string_view const title)
        : m_title(title)
    {
    }
    ApplicationSingleInstance::ApplicationSingleInstance(std::string_view const title, std::string_view const uuid)
        : m_title(title)
    {
        Initialize(uuid);
    }
