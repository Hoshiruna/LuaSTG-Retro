...
namespace Platform
{
    struct Lock
    {
        struct ScopeLock
        {
            ...
            ScopeLock(CRITICAL_SECTION& sec)
                : sec_(sec)
            {
    ...
    HResultToBool& HResultToBool::operator=(HResult v) noexcept
    {
        hr = v;
        return *this;
    }
    HResultToBool::operator bool() const noexcept
    {
        return SUCCEEDED(hr);
    }
    HResultToBool::HResultToBool() noexcept
        : hr(S_OK)
    {
    }
    HResultToBool::HResultToBool(HResult const hr_) noexcept
        : hr(hr_)
    {
    }
}
