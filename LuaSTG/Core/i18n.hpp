...
inline void
i18n_log_info_fmt(T&& fmt, A&&... args)
{
...
template<typename T, typename... A>
inline void
i18n_log_warn_fmt(T&& fmt, A&&... args)
{
...
template<typename T, typename... A>
inline void
i18n_log_error_fmt(T&& fmt, A&&... args)
{
...
template<typename T, typename... A>
inline void
i18n_log_critical_fmt(T&& fmt, A&&... args)
{
    spdlog::critical(fmt::runtime(i18n(std::forward<T>(fmt))), std::forward<A>(args)...);
}

inline void
i18n_core_system_call_report_error(std::string_view message)
{
