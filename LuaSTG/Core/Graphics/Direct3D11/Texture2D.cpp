    ...
    bool Texture2D::createResource()
    {
        ...
        if(m_data) {
            ...
            HRESULT const hr1 = DirectX::CreateDDSTextureFromMemoryEx(
                d3d11_device, m_mipmap ? d3d11_devctx : nullptr, static_cast<uint8_t const*>(src->data()), src->size(), 0, D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE, 0, 0,
            ...
            if(FAILED(hr1)) {
                ...
                HRESULT const hr2 = DirectX::CreateWICTextureFromMemoryEx(
                    d3d11_device, m_mipmap ? d3d11_devctx : nullptr, static_cast<uint8_t const*>(src->data()), src->size(), 0, D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE, 0, 0, DirectX::WIC_LOADER_DEFAULT | DirectX::WIC_LOADER_IGNORE_SRGB,
                ...
                if(FAILED(hr2)) {
                    ...
                    HRESULT const hr3 = DirectX::CreateQOITextureFromMemoryEx(
                        d3d11_device, m_mipmap ? d3d11_devctx : nullptr, m_device->GetWICImagingFactory(), static_cast<uint8_t const*>(src->data()), src->size(), 0, D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE, 0, 0, DirectX::QOI_LOADER_DEFAULT | DirectX::QOI_LOADER_IGNORE_SRGB,
                        // TODO: 渲染管线目前是在 sRGB 下计算的，也就是心理视觉色彩，将错就错吧……
