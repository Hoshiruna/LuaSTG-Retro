...
#define HRGet hr = gHR
#define HRCheckCallReport(x)                   \
    if(FAILED(hr)) {                           \
        i18n_core_system_call_report_error(x); \
    }
...
#define HRCheckCallNoAssertReturnBool(x)       \
    if(FAILED(hr)) {                           \
        i18n_core_system_call_report_error(x); \
        return false;                          \
    }
...
#define NTGet nt
#define NTCheckCallReport(x)                   \
    if(nt != STATUS_SUCCESS) {                 \
        i18n_core_system_call_report_error(x); \
    }
#define NTCheckCallReturnBool(x)               \
    if(nt != STATUS_SUCCESS) {                 \
        i18n_core_system_call_report_error(x); \
        assert(false);                         \
        return false;                          \
    }
#define NTCheckCallNoAssertReturnBool(x)       \
    if(nt != STATUS_SUCCESS) {                 \
        i18n_core_system_call_report_error(x); \
        return false;                          \
    }
...
namespace core::Graphics
{
    ...
    static bool findBestDisplayMode(HWND const window, IDXGISwapChain1* dxgi_swapchain, Vector2U canvas_size, DXGI_MODE_DESC1& mode)
    {
        ...
        auto checkRequiredAspectRatio = [&](DXGI_MODE_DESC1 const& v) -> bool {
            if(monitor_w > monitor_h) {
                ...
                if(width == 0)
                    ...
                    return true;
                else if(width > v.Width && (width - v.Width) <= 2)
                    return true;
                else if(v.Width > width && (v.Width - width) <= 2)
                    return true;
                else
                    return false;
            } else {
                ...
                if(height == 0)
                    ...
                    return true;
                else if(height > v.Height && (height - v.Height) <= 2)
                    return true;
                else if(v.Height > height && (v.Height - height) <= 2)
                    return true;
                else
                    return false;
            }
        ...
        for(size_t i = 0; i < mode_list.size(); i += 1) {
            spdlog::info("{: >4d}: ({: >5d} x {: >5d}) {:.2f}Hz {} {}", i, mode_list[i].Width, mode_list[i].Height, (double)mode_list[i].RefreshRate.Numerator / (double)mode_list[i].RefreshRate.Denominator, to_string(mode_list[i].Scaling), to_string(mode_list[i].ScanlineOrdering));
        }
    ...
    {
        return info.SwapEffect == DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL || info.SwapEffect == DXGI_SWAP_EFFECT_FLIP_DISCARD;
    }
    ...
    bool SwapChain_D3D11::createSwapChain(bool fullscreen, DXGI_MODE_DESC1 const& mode, bool no_attachment)
    {
        ...
        if(!fullscreen)
            refresh_rate_string = i18n("DXGI.DisplayMode.RefreshRate.Desktop");
        std::string_view swapchain_model = i18n("DXGI.SwapChain.SwapEffect.Discard");
        if(m_swap_chain_info.SwapEffect == DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL)
            swapchain_model = i18n("DXGI.SwapChain.SwapEffect.FlipSequential");
        if(m_swap_chain_info.SwapEffect == DXGI_SWAP_EFFECT_FLIP_DISCARD)
            swapchain_model = i18n("DXGI.SwapChain.SwapEffect.FlipDiscard");
        auto enable_or_disable = [](bool v) -> std::string_view { return v ? i18n("Enable") : i18n("Disable"); };
        i18n_log_info_fmt("[core].SwapChain_D3D11.created_swapchain_info_fmt", mode.Width, mode.Height, refresh_rate_string, enable_or_disable(fullscreen), swapchain_model, enable_or_disable(m_swap_chain_info.Flags & DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING), enable_or_disable(m_swap_chain_info.Flags & DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT));
    ...
    bool SwapChain_D3D11::createDirectCompositionResources()
    {
        ...
        if(m_is_composition_mode) {
            ...
            if(!updateDirectCompositionTransform())
                return false;
        } else {
    ...
    bool SwapChain_D3D11::updateDirectCompositionTransform()
    {
        ...
        HRGet = dcomp_visual_swap_chain->SetBitmapInterpolationMode(
            layout.point_sampler ? DCOMPOSITION_BITMAP_INTERPOLATION_MODE_NEAREST_NEIGHBOR : DCOMPOSITION_BITMAP_INTERPOLATION_MODE_LINEAR);
        HRCheckCallReturnBool("IDCompositionVisual2::SetBitmapInterpolationMode");
    ...
    bool SwapChain_D3D11::createCompositionSwapChain(Vector2U size, bool latency_event)
    {
        ...
        if(m_swap_chain_info.SwapEffect == DXGI_SWAP_EFFECT_FLIP_DISCARD)
            swapchain_model = i18n("DXGI.SwapChain.SwapEffect.FlipDiscard");
        auto enable_or_disable = [](bool v) -> std::string_view { return v ? i18n("Enable") : i18n("Disable"); };
        i18n_log_info_fmt("[core].SwapChain_D3D11.created_swapchain_info_fmt", size.x, size.y, refresh_rate_string, i18n("Disable") // 没有独占全屏
            ,
            swapchain_model,
    ...
    bool SwapChain_D3D11::createRenderAttachment()
    {
        ...
        if(m_is_composition_mode) {
            ...
            if(!createCanvasColorBuffer())
                return false;
        }
        if(!createCanvasDepthStencilBuffer())
            return false;
        return true;
    }
