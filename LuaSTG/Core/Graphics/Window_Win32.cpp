...
namespace core::Graphics
{
    ...
    Display_Win32::Display_Win32(HMONITOR const monitor)
...
#define APPMODEL ((ApplicationModel_Win32*)m_framework)
    ...
    LRESULT Window_Win32::onMessage(HWND window, UINT message, WPARAM arg1, LPARAM arg2)
    {
        if(enable_track_window_focus && win32_window_want_track_focus) {
            ...
            if(focus_window && focus_window != window) {
                ...
                spdlog::info("[core] 窗口焦点已改变，新的焦点窗口为：[{}] ({}) {}", (void*)focus_window, utf8::to_string(std::wstring_view(buffer2.data(), (size_t)length2)), utf8::to_string(std::wstring_view(buffer1.data(), (size_t)length1)));
            }
        }
    ...
    void Window_Win32::_setWindowMode(SetWindowedModeParameters* parameters, bool ignore_size)
    {
        ...
        assert(get_monitor_info_result);
        (void)get_monitor_info_result;
        ...
        m_title_bar_controller.adjustWindowRectExForDpi(
            &rect, new_win32_window_style, FALSE, 0, win32::getDpiForWindow(win32_window));
        ...
        assert(set_style_result == 0);
        (void)set_style_result;
        //SetLastError(0);
    ...
    void Window_Win32::_setFullScreenMode(IDisplay* display)
    {
        ...
        if(!m_fullscreen_mode) {
            ...
            assert(get_placement_result);
            (void)get_placement_result;
        }
    ...
    {
        return SetWindowPos(win32_window, NULL, v.a.x, v.a.y, v.b.x - v.a.x, v.b.y - v.a.y, SWP_NOZORDER) != FALSE;
    }
    ...
    bool Window_Win32::setClientRect(RectI v)
    {
        ...
        // 最后再应用
        return SetWindowPos(win32_window, NULL, rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top, SWP_NOZORDER) != FALSE;
    }
    ...
    void Window_Win32::dispatchEvent(EventType t, EventData d)
    {
        ...
        switch(t) {
            ...
            case EventType::WindowSize:
                for(auto& v : m_eventobj) {
                    if(v)
                        v->onWindowSize(d.window_size);
                }
                break;
            case EventType::WindowFullscreenStateChange:
                for(auto& v : m_eventobj) {
                    if(v)
                        v->onWindowFullscreenStateChange(d.window_fullscreen_state);
                }
                break;
    ...
    void* Window_Win32::getNativeHandle()
    {
        return win32_window;
    }
