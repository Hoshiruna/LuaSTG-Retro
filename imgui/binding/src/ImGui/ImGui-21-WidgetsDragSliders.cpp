...
namespace
{
    ...
    int DragScalarT(lua_State* const vm)
    {
        ...
        auto const result = ImGui::DragScalar(label.data(), DataType, &p_data, v_speed, ctx.is_non_or_nil(5) ? nullptr : &p_min, ctx.is_non_or_nil(6) ? nullptr : &p_max, format.data(), flags);
        ctx.push_value(result);
    ...
    int DragScalarNT(lua_State* const vm)
    {
        ...
        auto const flags = ctx.get_value<ImGuiSliderFlags>(9, 0);
        auto const result = ImGui::DragScalarN(label.data(), DataType, data.data(), components, v_speed, ctx.is_non_or_nil(6) ? nullptr : &p_min, ctx.is_non_or_nil(7) ? nullptr : &p_max, format.data(), flags);
        for(int i = 0; i < components; i += 1) {
