...
namespace
{
    ...
    int InputScalarT(lua_State* const vm)
    {
        ...
        auto const result = ImGui::InputScalar(label.data(), DataType, &p_data, ctx.is_non_or_nil(4) ? nullptr : &p_step, ctx.is_non_or_nil(5) ? nullptr : &p_step_fast, format.data(), flags);
        ctx.push_value(result);
    ...
    int InputScalarNT(lua_State* const vm)
    {
        ...
        auto const result = ImGui::InputScalarN(label.data(), DataType, data.data(), components, ctx.is_non_or_nil(4) ? nullptr : &p_step, ctx.is_non_or_nil(5) ? nullptr : &p_step_fast, format.data(), flags);
        for(int i = 0; i < components; i += 1) {
