...
#define GET_FLAG(name)                                    \
    if(key == (#name ""sv)) {                             \
        ctx.push_value(static_cast<int32_t>(self->name)); \
        return 1;                                         \
    }                                                     \
    (void)0
#define GET_SCALAR(name)            \
    ...
    (void)0
#define GET_VEC2(name)                         \
    if(key == (#name ""sv)) {                  \
        ImVec2Binding::create(vm, self->name); \
        return 1;                              \
    }                                          \
    (void)0
#define GET_COLOR(name)                                           \
    if(key == ("Colors" #name ""sv)) {                            \
        ImVec4Binding::create(vm, self->Colors[ImGuiCol_##name]); \
        return 1;                                                 \
    }                                                             \
    (void)0
...
#define SET_FLAG(name)                                                             \
    if(key == (#name ""sv)) {                                                      \
        self->name = static_cast<decltype(self->name)>(ctx.get_value<int32_t>(3)); \
        return 0;                                                                  \
    }                                                                              \
    (void)0
#define SET_SCALAR(name)                                     \
    if(key == (#name ""sv)) {                                \
        self->name = ctx.get_value<decltype(self->name)>(3); \
        return 0;                                            \
    }                                                        \
    (void)0
#define SET_VEC2(name)                          \
    if(key == (#name ""sv)) {                   \
        self->name = *ImVec2Binding::as(vm, 3); \
        return 0;                               \
    }                                           \
    (void)0
#define SET_COLOR(name)                                            \
    if(key == ("Colors" #name ""sv)) {                             \
        self->Colors[ImGuiCol_##name] = *ImVec4Binding::as(vm, 3); \
        return 0;                                                  \
    }                                                              \
    (void)0
