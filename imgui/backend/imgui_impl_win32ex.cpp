...
ImGui_ImplWin32Ex_GetMouseSourceFromMessageExtraInfo()
{
...
struct Win32MessageQueueSafe
{
    ...
    ~Win32MessageQueueSafe()
    {
        ...
        reader_index = 0;
        if(semaphore_space)
            ::CloseHandle(semaphore_space);
        semaphore_space = NULL;
        if(semaphore_data)
            ::CloseHandle(semaphore_data);
        semaphore_data = NULL;
    }
};
...
static ImGui_ImplWin32Ex_Data*
ImGui_ImplWin32Ex_GetBackendData()
{
...
static void
ImGui_ImplWin32Ex_AddKeyEvent(ImGuiKey key, bool down, int native_keycode, int native_scancode = -1)
{
...
ImGuiKey
ImGui_ImplWin32Ex_KeyEventToImGuiKey(WPARAM wParam, LPARAM lParam)
{
...
static bool
ImGui_ImplWin32Ex_MapMouseCursor(LPWSTR* outValue)
{
    ImGuiIO& io = ImGui::GetIO();
...
static void
ImGui_ImplWin32Ex_UpdateIME(ImGuiContext*, ImGuiViewport* viewport, ImGuiPlatformImeData* data)
{
...
static void
ImGui_ImplWin32Ex_ProcessMessage(ImGui_ImplWin32Ex_FrameData* frame_data)
{
    ImGui_ImplWin32Ex_Data* bd = ImGui_ImplWin32Ex_GetBackendData();
    ImGuiIO& io = ImGui::GetIO();
    Win32Message msg = {};
    ...
    auto processInputEvent = [&]() -> bool {
        ...
        switch(msg.uMsg) {
            ...
            case WM_XBUTTONDBLCLK: {
                ...
                if(msg.uMsg == WM_RBUTTONDOWN || msg.uMsg == WM_RBUTTONDBLCLK) {
                    button = 1;
                }
                if(msg.uMsg == WM_MBUTTONDOWN || msg.uMsg == WM_MBUTTONDBLCLK) {
                    button = 2;
                }
                if(msg.uMsg == WM_XBUTTONDOWN || msg.uMsg == WM_XBUTTONDBLCLK) {
                ...
                if(msg.wParam < 256) {
                    ...
                    if(vk == VK_SHIFT) {
                        ...
                        if(IsVkDown(VK_LSHIFT) == is_key_down) {
                            ImGui_ImplWin32Ex_AddKeyEvent(ImGuiKey_LeftShift, is_key_down, VK_LSHIFT, scancode);
                        }
                        if(IsVkDown(VK_RSHIFT) == is_key_down) {
                            ImGui_ImplWin32Ex_AddKeyEvent(ImGuiKey_RightShift, is_key_down, VK_RSHIFT, scancode);
                        }
                    } else if(vk == VK_CONTROL) {
                        if(IsVkDown(VK_LCONTROL) == is_key_down) {
                            ImGui_ImplWin32Ex_AddKeyEvent(ImGuiKey_LeftCtrl, is_key_down, VK_LCONTROL, scancode);
                        }
                        if(IsVkDown(VK_RCONTROL) == is_key_down) {
                            ImGui_ImplWin32Ex_AddKeyEvent(ImGuiKey_RightCtrl, is_key_down, VK_RCONTROL, scancode);
                        }
                    } else if(vk == VK_MENU) {
                        if(IsVkDown(VK_LMENU) == is_key_down) {
                            ImGui_ImplWin32Ex_AddKeyEvent(ImGuiKey_LeftAlt, is_key_down, VK_LMENU, scancode);
                        }
                        if(IsVkDown(VK_RMENU) == is_key_down) {
                            ImGui_ImplWin32Ex_AddKeyEvent(ImGuiKey_RightAlt, is_key_down, VK_RMENU, scancode);
                        }
                    }
                }
            }
...
bool
ImGui_ImplWin32Ex_Init(void* window)
{
    ImGuiIO& io = ImGui::GetIO();
    IMGUI_CHECKVERSION();
...
void
ImGui_ImplWin32Ex_NewFrame(ImGui_ImplWin32Ex_FrameData* frame_data)
{
    ImGui_ImplWin32Ex_Data* bd = ImGui_ImplWin32Ex_GetBackendData();
...
ImGui_ImplWin32Ex_WndProcHandler(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
