...
namespace utf
{
    ...
    namespace detail
    {
        ...
        {
            return (str[0] & 0xE0) == 0xC0 && (str[1] & 0xC0) == 0x80;
        }
        inline bool is_utf8_l3(char const* str)
        {
            return (str[0] & 0xF0) == 0xE0 && (str[1] & 0xC0) == 0x80 && (str[2] & 0xC0) == 0x80;
        }
        inline bool is_utf8_l4(char const* str)
        {
            return (str[0] & 0xF8) == 0xF0 && (str[1] & 0xC0) == 0x80 && (str[2] & 0xC0) == 0x80 && (str[3] & 0xC0) == 0x80;
        }
        ...
        {
            return char32_t(
                ((char32_t(str[0]) & 0x1F) << 6) | (char32_t(str[1]) & 0x3F));
        }
        inline char32_t utf8_to_utf32_l3(char const* str)
        {
            return char32_t(
                ((char32_t(str[0]) & 0x0F) << 12) | ((char32_t(str[1]) & 0x3F) << 6) | (char32_t(str[2]) & 0x3F));
        }
        inline char32_t utf8_to_utf32_l4(char const* str)
        {
            return char32_t(
                ((char32_t(str[0]) & 0x07) << 18) | ((char32_t(str[1]) & 0x3F) << 12) | ((char32_t(str[2]) & 0x3F) << 6) | (char32_t(str[3]) & 0x3F));
        }
    }
    ...
    public:
        utf8reader(char const* str, size_t len)
            : _raw_str(str), _raw_len(len), _str(str), _len(len)
        {
        }
        utf8reader(char const* str)
            : _raw_str(str), _raw_len(0), _str(str), _len(0)
        {
            // warning: unsafe!!!
    ...
    public:
        utf16reader(char16_t const* str, size_t len)
            : _str(str), _len(len) {}
        utf16reader(char16_t const* str)
            : _str(str), _len(0)
        {
            // warning: unsafe!!!
            while(_str[_len] != 0) {
