...
namespace core::Graphics
{
    ...
    {
        // upload data
        if((_draw_list.vertex.capacity - _vi_buffer[_vi_buffer_index].vertex_offset) < _draw_list.vertex.size || (_draw_list.index.capacity - _vi_buffer[_vi_buffer_index].index_offset) < _draw_list.index.size) {
            // next  buffer
    ...
    bool Renderer_D3D11::batchFlush(bool discard)
    {
        ...
        if(!discard) {
            ...
            // upload data
            if(!uploadVertexIndexBufferFromDrawList())
                return false;
            // draw
            auto* ctx = m_device->GetD3D11DeviceContext();
            assert(ctx);
