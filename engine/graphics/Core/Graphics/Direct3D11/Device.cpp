...
namespace core::Graphics::Direct3D11
{
    ...
    bool Device::selectAdapter()
    {
        ...
        for(UINT idx = 0; bHR = dxgi_factory->EnumAdapters1(idx, &dxgi_adapter_temp); idx += 1) {
            ...
            if(bHR = gHR = dxgi_adapter_temp->GetDesc1(&desc_)) {
                ...
                i18n_log_info_fmt("[core].Device_D3D11.DXGI_adapter_detail_fmt", idx, dev_name, d3d_feature_level_to_string(level_info), adapter_flags_to_string(desc_.Flags), soft_dev_type ? i18n("DXGI_adapter_type_software_warning") : "", bytes_count_to_string(desc_.DedicatedVideoMemory), bytes_count_to_string(desc_.DedicatedSystemMemory), bytes_count_to_string(desc_.SharedSystemMemory), desc_.VendorId, desc_.DeviceId, desc_.SubSysId, desc_.Revision, static_cast<DWORD>(desc_.AdapterLuid.HighPart), desc_.AdapterLuid.LowPart);
                if(soft_dev_type) {
    ...
    bool Device::createDXGI()
    {
        ...
        // 打印特性支持情况
        i18n_log_info_fmt("[core].Device_D3D11.DXGI_detail_fmt", dxgi_support_tearing ? i18n("support") : i18n("not_support.requires_Windows_10_and_hardware"));
    ...
    bool Device::createD3D11()
    {
        ...
        auto check_format_support = [&](DXGI_FORMAT const format, std::string_view const& name) -> D3DX11_FEATURE_DATA_FORMAT_SUPPORT {
            std::string name1("ID3D11Device::CheckFeatureSupport -> D3D11_FEATURE_FORMAT_SUPPORT (");
            name1.append(name);
            name1.append(")");
            std::string name2("ID3D11Device::CheckFeatureSupport -> D3D11_FEATURE_FORMAT_SUPPORT2 (");
            name2.append(name);
            name2.append(")");
            ...
            if(FAILED(hr1))
                i18n_core_system_call_report_error(name1);
            ...
            if(FAILED(hr2))
                i18n_core_system_call_report_error(name2);
...
#undef _FORMAT_INFO_STRING_FMT3
        ...
        if(
            (d3d11_feature_format_bgra32.OutFormatSupport & D3D11_FORMAT_SUPPORT_TEXTURE2D) && (d3d11_feature_format_bgra32.OutFormatSupport & D3D11_FORMAT_SUPPORT_IA_VERTEX_BUFFER) && (d3d11_feature_format_bgra32.OutFormatSupport & D3D11_FORMAT_SUPPORT_MIP) && (d3d11_feature_format_bgra32.OutFormatSupport & D3D11_FORMAT_SUPPORT_RENDER_TARGET) && (d3d11_feature_format_bgra32.OutFormatSupport & D3D11_FORMAT_SUPPORT_BLENDABLE) && (d3d11_feature_format_bgra32.OutFormatSupport & D3D11_FORMAT_SUPPORT_DISPLAY)) {
            // 确实支持
        } else {
    ...
    bool Device::testAdapterPolicy()
    {
        ...
        if(testStage(false, false))
            return true;
        // Stage 2 - Enable and test
        if(testStage(true, true))
            return true;
        // Stage 3 - NVIDIA and test
        if(testStage(true, false))
            return true;
        // Stage 4 - AMD and test
        if(testStage(false, true))
            return true;
