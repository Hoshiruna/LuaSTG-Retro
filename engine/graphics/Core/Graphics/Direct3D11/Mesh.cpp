    ...
    bool compileVertexShader(std::string const& source, ID3DBlob** const output)
    {
        *output = nullptr;
        ...
        hr = gHR = d3d_compiler.Compile(
            source.data(), source.size(), "Mesh generated vertex shader", nullptr, nullptr, "main", "vs_4_0", flags, 0, vertex_shader_blob.put(), error_message_blob.put());
        if(FAILED(hr)) {
...
#define REPORT_VERTEX_INDEX_OOB_RETURN(v)                 \
    if(m_validation && ((v) >= m_options.vertex_count)) { \
        reportIndexOutOfBounds();                         \
        assert(false);                                    \
        return;                                           \
    }

#define REPORT_INDEX_INDEX_OOB_RETURN(v)                 \
    if(m_validation && ((v) >= m_options.index_count)) { \
        reportIndexOutOfBounds();                        \
        assert(false);                                   \
        return;                                          \
    }

#define REPORT_READ_ONLY_RETURN       \
    if(m_validation && m_read_only) { \
        reportReadOnly();             \
        assert(false);                \
        return;                       \
    }

#define REPORT_READ_ONLY_RETURN_BOOL  \
    if(m_validation && m_read_only) { \
        reportReadOnly();             \
        assert(false);                \
        return false;                 \
...
namespace core::Graphics::Direct3D11
{
    ...
    void Mesh::applyNative(ID3D11DeviceContext* const ctx, bool const fog)
    {
        ...
        ctx->IASetPrimitiveTopology(m_options.primitive_topology == PrimitiveTopology::triangle_list ? D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST : D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
