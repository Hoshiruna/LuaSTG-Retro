...
namespace core::Graphics::Common
{
        ...
        if(m_glyph_mgr->getTextureCount() <= 1) {
            ...
            while(reader_(code_)) {
                ...
                if(m_glyph_mgr->getGlyph(code_, &glyph_info, true)) // 不要在这里 flush
                {
                    ...
                    if(!drawGlyph(glyph_info, start_pos))
                        return false;
        ...
        if(m_glyph_mgr->getTextureCount() <= 1) {
            ...
            while(reader_(code_)) {
                ...
                if(m_glyph_mgr->getGlyph(code_, &glyph_info, true)) // 不要在这里 flush
                {
                    ...
                    glyph_info.position.x *= m_scale.x;
                    glyph_info.position.y *= m_scale.y;
                    if(!drawGlyphInSpace(glyph_info, start_pos, right_vec, down_vec))
                        return false;
    ...
    TextRenderer::TextRenderer(IRenderer* const p_renderer)
        : m_renderer(p_renderer), m_scale(Vector2F(1.0f, 1.0f)), m_z(0.5f), m_color(Color4B(0xFFFFFFFFu))
    {
    }
