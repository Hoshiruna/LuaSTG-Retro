...
namespace d3d11
{
    ...
    bool findBestDisplayMode(IDXGISwapChain1* const swap_chain, const HMONITOR display, const UINT target_width, const UINT target_height, DXGI_MODE_DESC1& mode)
    {
        ...
        for(size_t i = 0; i < mode_list.size(); i += 1) {
            core::Logger::info("{: >4d}: ({: >5d} x {: >5d}) {:.2f}Hz {} {}", i, mode_list[i].Width, mode_list[i].Height, (double)mode_list[i].RefreshRate.Numerator / (double)mode_list[i].RefreshRate.Denominator, toStringView(mode_list[i].Scaling), toStringView(mode_list[i].ScanlineOrdering));
        }
