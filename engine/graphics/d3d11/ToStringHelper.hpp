...
namespace
{
    ...
    {
        std::string s;
        if(v & DXGI_ADAPTER_FLAG3_REMOTE)
            s.append("REMOTE "sv);
        if(v & DXGI_ADAPTER_FLAG3_SOFTWARE)
            s.append("SOFTWARE "sv);
        if(v & DXGI_ADAPTER_FLAG3_ACG_COMPATIBLE)
            s.append("ACG_COMPATIBLE "sv);
        if(v & DXGI_ADAPTER_FLAG3_SUPPORT_MONITORED_FENCES)
            s.append("SUPPORT_MONITORED_FENCES "sv);
        if(v & DXGI_ADAPTER_FLAG3_SUPPORT_NON_MONITORED_FENCES)
            s.append("SUPPORT_NON_MONITORED_FENCES "sv);
        if(v & DXGI_ADAPTER_FLAG3_KEYED_MUTEX_CONFORMANCE)
            s.append("KEYED_MUTEX_CONFORMANCE "sv);
        if(!s.empty())
            if(s.back() == ' ')
                s.pop_back();
        return s;
