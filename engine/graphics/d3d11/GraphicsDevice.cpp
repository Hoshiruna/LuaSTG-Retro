...
namespace core
{
    ...
    bool GraphicsDevice::testAdapterPolicy()
    {
        ...
        if(testStage(preferred_adapter_name, false, false))
            return true;
        // Stage 2 - Enable and test
        if(testStage(preferred_adapter_name, true, true))
            return true;
        // Stage 3 - NVIDIA and test
        if(testStage(preferred_adapter_name, true, false))
            return true;
        // Stage 4 - AMD and test
        if(testStage(preferred_adapter_name, false, true))
            return true;
    ...
    bool GraphicsDevice::createD3D11()
    {
        ...
        if(dxgi_adapter) {
            ...
            if(d3d11::createSoftwareDevice(
                   d3d11_device.put(), d3d11_devctx.put(), &d3d_feature_level, &driver_type)) {
        ...
        constexpr UINT required_support = D3D11_FORMAT_SUPPORT_TEXTURE2D | D3D11_FORMAT_SUPPORT_IA_VERTEX_BUFFER | D3D11_FORMAT_SUPPORT_MIP | D3D11_FORMAT_SUPPORT_RENDER_TARGET | D3D11_FORMAT_SUPPORT_BLENDABLE | D3D11_FORMAT_SUPPORT_DISPLAY;
