...
#define NTCheckCallReportRuntime(x)                       \
    if(nt != STATUS_SUCCESS) {                            \
        core::Logger::error("Windows API failed: {}", x); \
    }
...
namespace
{
    ...
    bool setFullscreenState(IDXGISwapChain* const swap_chain, const BOOL fullscreen)
    {
        ...
        return win32::check_hresult_as_boolean(hr, fullscreen ? "IDXGISwapChain::SetFullscreenState (TRUE)"sv : "IDXGISwapChain::SetFullscreenState (FALSE)"sv);
    }
}
