...
namespace
{
    ...
    bool isSoftwareOrRemoteAdapter(const DXGI_ADAPTER_DESC3& info)
    {
        ...
        return ((info.VendorId == 0x1414) && (info.DeviceId == 0x8c)) || (info.Flags & DXGI_ADAPTER_FLAG3_REMOTE) || (info.Flags & DXGI_ADAPTER_FLAG3_SOFTWARE);
    }
    ...
    D3D_FEATURE_LEVEL testAdapterFeatureLevel(win32::com_ptr<IDXGIAdapter1>& adapter)
    {
        ...
        for(UINT offset = 0; offset <= 3; offset += 1) {
            const auto hr = D3D11CreateDevice(
                adapter.get(), D3D_DRIVER_TYPE_UNKNOWN, nullptr, flags, feature_levels + offset, 9u - offset, D3D11_SDK_VERSION, nullptr, &feature_level, nullptr);
            if(SUCCEEDED(hr)) {
                break;
            }
        }
    ...
    bool internalRefresh(win32::com_ptr<IDXGIFactory2>& factory, const bool save_adapter)
    {
        ...
        for(UINT i = 0;; i += 1) {
            ...
            if(adapter4 && win32::check_hresult_as_boolean(adapter4->GetDesc3(&info.info), "IDXGIAdapter4::GetDesc3"sv)) {
                info.info_level = 4;
            }

            static_assert(sizeof(DXGI_ADAPTER_DESC2) == sizeof(DXGI_ADAPTER_DESC3), "bad reinterpret_cast");
            if(info.info_level == 0 && adapter2 && win32::check_hresult_as_boolean(adapter2->GetDesc2(reinterpret_cast<DXGI_ADAPTER_DESC2*>(&info.info)), "IDXGIAdapter2::GetDesc2"sv)) {
                info.info_level = 2;
            }

            static_assert(sizeof(DXGI_ADAPTER_DESC1) == sizeof(DXGI_ADAPTER_DESC3) - 8, "bad reinterpret_cast");
            if(info.info_level == 0 && win32::check_hresult_as_boolean(adapter->GetDesc1(reinterpret_cast<DXGI_ADAPTER_DESC1*>(&info.info)), "IDXGIAdapter1::GetDesc1"sv)) {
                info.info_level = 1;
            }
    ...
    void logCollection(const std::vector<PhysicalDeviceInfo>& collection, std::string& s)
    {
        for(const auto& v : collection) {
            s.append(std::format(
                ...
                v.info.SubSysId,
                v.info.Revision,
                v.info.DedicatedVideoMemory,
                toString(v.info.DedicatedVideoMemory, Semantic::data_size),
                v.info.DedicatedSystemMemory,
                toString(v.info.DedicatedSystemMemory, Semantic::data_size),
                v.info.SharedSystemMemory,
                toString(v.info.SharedSystemMemory, Semantic::data_size),
                static_cast<DWORD>(v.info.AdapterLuid.HighPart),
                v.info.AdapterLuid.LowPart,
