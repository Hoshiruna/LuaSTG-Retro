...
namespace d3d11
{
    ...
    bool createDevice(
        ...
        D3D_FEATURE_LEVEL* const feature_level)
    {
        ...
        for(UINT offset = 0; offset <= 3; offset += 1) {
            const auto hr = D3D11CreateDevice(
                adapter, driver_type, nullptr, flags, feature_levels + offset, 9u - offset, D3D11_SDK_VERSION, device, feature_level, device_context);
            if(SUCCEEDED(hr)) {
                return true;
            }
        ...
        D3D_FEATURE_LEVEL* const feature_level)
    {
        ...
        for(UINT offset = 0; offset <= 3; offset += 1) {
            ...
            const auto hr = D3D11CreateDevice(
                nullptr, driver_type, nullptr, flags, feature_levels + offset, 9u - offset, D3D11_SDK_VERSION, local_devcie.put(), &local_feature_level, local_device_context.put());
            if(SUCCEEDED(hr) && local_feature_level >= D3D_FEATURE_LEVEL_10_0) {
                if(device)
                    *device = local_devcie.detach();
                if(device_context)
                    *device_context = local_device_context.detach();
                if(feature_level)
                    *feature_level = local_feature_level;
                return true;
            }
            results.push_back(hr);
        }
    ...
    bool checkFormatSupport(ID3D11Device* const device, const DXGI_FORMAT format, UINT* const support, UINT* const support2)
    {
        ...
        if(support)
            *support = info.OutFormatSupport;
        if(support2)
            *support2 = info2.OutFormatSupport2;
        return true;
    }
...
#undef GET_FEATURE
    ...
    void logDeviceFormatSupportDetail(const D3D11_FEATURE_DATA_FORMAT_SUPPORT& info, const D3D11_FEATURE_DATA_FORMAT_SUPPORT2& info2, const std::string_view name, std::string& s)
    {
        ...
        if(info.OutFormatSupport != 0) {
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_BUFFER)
                s.append(" BUFFER"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_IA_VERTEX_BUFFER)
                s.append(" IA_VERTEX_BUFFER"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_IA_INDEX_BUFFER)
                s.append(" IA_INDEX_BUFFER"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_SO_BUFFER)
                s.append(" SO_BUFFER"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_TEXTURE1D)
                s.append(" TEXTURE1D"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_TEXTURE2D)
                s.append(" TEXTURE2D"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_TEXTURE3D)
                s.append(" TEXTURE3D"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_TEXTURECUBE)
                s.append(" TEXTURECUBE"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_SHADER_LOAD)
                s.append(" SHADER_LOAD"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_SHADER_SAMPLE)
                s.append(" SHADER_SAMPLE"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_SHADER_SAMPLE_COMPARISON)
                s.append(" SHADER_SAMPLE_COMPARISON"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_SHADER_SAMPLE_MONO_TEXT)
                s.append(" SHADER_SAMPLE_MONO_TEXT"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_MIP)
                s.append(" MIP"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_MIP_AUTOGEN)
                s.append(" MIP_AUTOGEN"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_RENDER_TARGET)
                s.append(" RENDER_TARGET"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_BLENDABLE)
                s.append(" BLENDABLE"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_DEPTH_STENCIL)
                s.append(" DEPTH_STENCIL"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_CPU_LOCKABLE)
                s.append(" CPU_LOCKABLE"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_MULTISAMPLE_RESOLVE)
                s.append(" MULTISAMPLE_RESOLVE"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_DISPLAY)
                s.append(" DISPLAY"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_CAST_WITHIN_BIT_LAYOUT)
                s.append(" CAST_WITHIN_BIT_LAYOUT"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_MULTISAMPLE_RENDERTARGET)
                s.append(" MULTISAMPLE_RENDERTARGET"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_MULTISAMPLE_LOAD)
                s.append(" MULTISAMPLE_LOAD"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_SHADER_GATHER)
                s.append(" SHADER_GATHER"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_BACK_BUFFER_CAST)
                s.append(" BACK_BUFFER_CAST"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_TYPED_UNORDERED_ACCESS_VIEW)
                s.append(" TYPED_UNORDERED_ACCESS_VIEW"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_SHADER_GATHER_COMPARISON)
                s.append(" SHADER_GATHER_COMPARISON"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_DECODER_OUTPUT)
                s.append(" DECODER_OUTPUT"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_VIDEO_PROCESSOR_OUTPUT)
                s.append(" VIDEO_PROCESSOR_OUTPUT"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_VIDEO_PROCESSOR_INPUT)
                s.append(" VIDEO_PROCESSOR_INPUT"sv);
            if(info.OutFormatSupport & D3D11_FORMAT_SUPPORT_VIDEO_ENCODER)
                s.append(" VIDEO_ENCODER"sv);
        }
        if(info2.OutFormatSupport2 != 0) {
            if(info2.OutFormatSupport2 & D3D11_FORMAT_SUPPORT2_UAV_ATOMIC_ADD)
                s.append(" UAV_ATOMIC_ADD"sv);
            if(info2.OutFormatSupport2 & D3D11_FORMAT_SUPPORT2_UAV_ATOMIC_BITWISE_OPS)
                s.append(" UAV_ATOMIC_BITWISE_OPS"sv);
            if(info2.OutFormatSupport2 & D3D11_FORMAT_SUPPORT2_UAV_ATOMIC_COMPARE_STORE_OR_COMPARE_EXCHANGE)
                s.append(" UAV_ATOMIC_COMPARE_STORE_OR_COMPARE_EXCHANGE"sv);
            if(info2.OutFormatSupport2 & D3D11_FORMAT_SUPPORT2_UAV_ATOMIC_EXCHANGE)
                s.append(" UAV_ATOMIC_EXCHANGE"sv);
            if(info2.OutFormatSupport2 & D3D11_FORMAT_SUPPORT2_UAV_ATOMIC_SIGNED_MIN_OR_MAX)
                s.append(" UAV_ATOMIC_SIGNED_MIN_OR_MAX"sv);
            if(info2.OutFormatSupport2 & D3D11_FORMAT_SUPPORT2_UAV_ATOMIC_UNSIGNED_MIN_OR_MAX)
                s.append(" UAV_ATOMIC_UNSIGNED_MIN_OR_MAX"sv);
            if(info2.OutFormatSupport2 & D3D11_FORMAT_SUPPORT2_UAV_TYPED_LOAD)
                s.append(" UAV_TYPED_LOAD"sv);
            if(info2.OutFormatSupport2 & D3D11_FORMAT_SUPPORT2_UAV_TYPED_STORE)
                s.append(" UAV_TYPED_STORE"sv);
            if(info2.OutFormatSupport2 & D3D11_FORMAT_SUPPORT2_OUTPUT_MERGER_LOGIC_OP)
                s.append(" OUTPUT_MERGER_LOGIC_OP"sv);
            if(info2.OutFormatSupport2 & D3D11_FORMAT_SUPPORT2_TILED)
                s.append(" TILED"sv);
            if(info2.OutFormatSupport2 & D3D11_FORMAT_SUPPORT2_SHAREABLE)
                s.append(" SHAREABLE"sv);
            if(info2.OutFormatSupport2 & D3D11_FORMAT_SUPPORT2_MULTIPLANE_OVERLAY)
                s.append(" MULTIPLANE_OVERLAY"sv);
            if(info2.OutFormatSupport2 & D3D11_FORMAT_SUPPORT2_DISPLAYABLE)
                s.append(" DISPLAYABLE"sv);
        }
        s.push_back('\n');
    }
