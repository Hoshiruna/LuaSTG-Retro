...
namespace {
	...
	void listDevices() {
		if (!audio->refreshAudioDevices()) return;
	...
	int run() {
		if (!core::IAudioSystem::create(audio.put())) return 1;
		...
		while (true) {
			...
			std::print("> ");
			std::cin >> command;
			if (command == "exit"sv) break;
			if (command == "devices"sv) listDevices();
			else if (command == "device"sv) {
				uint32_t index{};
				std::cin >> index;
				if (index < audio->getAudioDeviceCount()) audio->setAudioDevice(audio->getAudioDevice(index).id);
			}
			...
			else if (command == "pause"sv) audio->pause(voice);
			else if (command == "resume"sv) audio->resume(voice);
			else if (command == "stop"sv) audio->stop(voice);
			else if (command == "state"sv) std::println("state = {}", static_cast<int>(audio->getVoiceState(voice)));
		}
		asset.reset();
		audio.reset();
		return 0;
