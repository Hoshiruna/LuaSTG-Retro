...
#define get_level(PATH)                                                            \
    ...
    if(s == "debug"sv) {                                                           \
        ...
        error_callback(std::format("[" PATH "] unknown logging level '{}'"sv, s)); \
        return false;                                                              \
    }
                ...
                if(file_system.contains("resources"sv)) {
                    ...
                    for(size_t i = 0; i < resources.size(); i += 1) {
                        ...
                        if(resource.contains("type"sv)) {
                            ...
                            if(s == "directory"sv) {
                                res_type = Type::directory;
                            } else if(s == "archive"sv) {
                                res_type = Type::archive;
                            } else {
                                error_callback(std::format("[/file_system/resources/{}/type] unknown resource type '{}'"sv, i, s));
                                return false;
                            }
