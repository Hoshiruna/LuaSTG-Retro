...
namespace core
{
    ...
    {
    public:
        [[nodiscard]] T& operator[](size_t const idx)
        {
            assert(!empty());
            return m_data[(idx + m_front) % MaxSize];
        }

        [[nodiscard]] T const& operator[](size_t const idx) const
        {
            assert(!empty());
            return m_data[(idx + m_front) % MaxSize];
        }
        ...
        void placementResize(size_t const size) noexcept
        {
            assert(size <= MaxSize);
            m_front = 0;
            m_back = size;
        ...
        [[nodiscard]] T& tail() noexcept
        {
            assert(!empty());
            return m_data[(m_back + MaxSize - 1) % MaxSize];
        }

        [[nodiscard]] T const& tail() const noexcept
        {
            assert(!empty());
            return m_data[(m_back + MaxSize - 1) % MaxSize];
        }

        [[nodiscard]] T& head() noexcept
        {
            assert(!empty());
            return m_data[m_front];
        }

        [[nodiscard]] T const& head() const noexcept
        {
            assert(!empty());
            return m_data[m_front];
        }
