...
namespace core
{
    ...
    struct FileNotifyInformation
    {
        ...
        FileNotifyInformation(FileNotifyInformation const& other)
            : file_name(other.file_name), action(other.action)
        {
        ...
        FileNotifyInformation(FileNotifyInformation&& other) noexcept
            : file_name(std::exchange(other.file_name, nullptr)), action(std::exchange(other.action, FileAction::unknown))
        {
        }
    ...
    template<>
    constexpr InterfaceId getInterfaceId<IMessageQueueBasedFileSystemWatcher>()
    {
        return UUID::parse("30b35341-08d1-5144-b3fe-7f96bd29978b");
    }
}
