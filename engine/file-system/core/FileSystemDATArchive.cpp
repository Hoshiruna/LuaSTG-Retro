...
namespace core
{
    ...
    namespace
    {
        ...
        {
            return path.size() >= 2 && path[1] == ':' && std::isalpha(static_cast<unsigned char>(path[0]));
        }
        ...
        static bool hasParentTraversal(std::string_view const& path)
        {
            ...
            for(;;) {
                ...
                std::string_view const segment = end == std::string_view::npos ? path.substr(begin) : path.substr(begin, end - begin);
                if(segment == "..") {
                    return true;
                }
        ...
        static bool normalizeArchiveFilePath(std::string& path)
        {
            ...
            return !path.empty() && path.front() != '/' && path.back() != '/' && !isDrivePath(path) && path.find('\0') == std::string::npos && !hasParentTraversal(path);
        }
        ...
        static bool readU32LE(uint8_t const*& cursor, uint8_t const* end, uint32_t& value)
        {
            ...
            value = static_cast<uint32_t>(cursor[0]) | (static_cast<uint32_t>(cursor[1]) << 8) | (static_cast<uint32_t>(cursor[2]) << 16) | (static_cast<uint32_t>(cursor[3]) << 24);
            cursor += sizeof(uint32_t);
            return true;
        }
        ...
        static bool readHeader(uint8_t const* data, size_t size, DATArchiveHeader& header)
        {
            ...
            cursor += DATArchiveEncryption::HEADER_MAGIC_LENGTH;
            return readU32LE(cursor, end, header.entryCount) && readU32LE(cursor, end, header.headerOffset) && readU32LE(cursor, end, header.headerSize) && cursor == end;
        }
        ...
        {
            if(entry.path.size() > std::numeric_limits<uint32_t>::max())
                return false;
        ...
        {
            uint32_t pathSize = 0;
            if(!readU32LE(cursor, end, pathSize))
                return false;
            if(pathSize == 0 || static_cast<size_t>(pathSize) > static_cast<size_t>(end - cursor))
                return false;

            entry.path.assign(reinterpret_cast<char const*>(cursor), pathSize);
            cursor += pathSize;
            if(!normalizeArchiveFilePath(entry.path))
                return false;

            uint8_t ct = 0;
            if(!readU8(cursor, end, ct))
                return false;
            entry.compressionType = static_cast<DATArchiveEntry::CompressionType>(ct);

            return readU32LE(cursor, end, entry.sizeFull) && readU32LE(cursor, end, entry.sizeStored) && readU32LE(cursor, end, entry.offsetPos) && readU8(cursor, end, entry.keyBase) && readU8(cursor, end, entry.keyStep) && readU32LE(cursor, end, entry.crc32Value);
        }
    ...
    bool FileSystemDATArchive::open(std::string_view const& path, size_t readOffset)
    {
        ...
        DATArchiveEncryption::getKeyHashHeader(masterKey, keyBase, keyStep);
        {
            ...
            if(readHeader(buf, sizeof(buf), header) && std::memcmp(header.magic, DATArchiveEncryption::HEADER_MAGIC, DATArchiveEncryption::HEADER_MAGIC_LENGTH) == 0) {
                m_keyBase = base;
                m_keyStep = keyStep;
            } else {
        ...
        for(uint32_t i = 0; i < header.entryCount; ++i) {
            ...
            if(entry.path.empty() || entry.compressionType > DATArchiveEntry::CT_ZLIB || !normalizeArchiveFilePath(entry.path) || !isRangeInside(m_readOffset, entry.offsetPos, entry.sizeStored, archiveSize) || (entry.compressionType == DATArchiveEntry::CT_NONE && entry.sizeFull != entry.sizeStored)) {
    ...
    std::string_view FileSystemDATArchive::getArchivePath()
    {
        return m_path;
    }
    ...
    bool FileSystemDATArchive::readEntryData(DATArchiveEntry const& entry, IData** const data)
    {
        ...
        if(!IData::create(outBuf.size(), result.put()))
            return false;
        if(!outBuf.empty())
    ...
    bool FileSystemDATArchive::isDATArchive(std::string_view const& path, size_t readOffset)
    {
        ...
        DATArchiveHeader header{};
        return readHeader(headerBuf, sizeof(headerBuf), header) && std::memcmp(header.magic, DATArchiveEncryption::HEADER_MAGIC, DATArchiveEncryption::HEADER_MAGIC_LENGTH) == 0;
    }
    ...
    bool FileSystemDATArchive::createFromFile(std::string_view const& path, size_t readOffset, IFileSystemArchive** const archive)
    {
        SmartReference<FileSystemDATArchive> object;
        object.attach(new FileSystemDATArchive());
        if(!object->open(path, readOffset))
            return false;
    ...
    FileSystemNodeType FileSystemDATArchiveEnumerator::getNodeType()
    {
        if(m_index < 0 || static_cast<size_t>(m_index) >= m_items.size())
            return FileSystemNodeType::unknown;
        return m_items[m_index].isDirectory ? FileSystemNodeType::directory : FileSystemNodeType::file;
    }
    ...
    bool FileSystemDATArchiveEnumerator::readFile(IData** const data)
    {
        if(m_index < 0 || static_cast<size_t>(m_index) >= m_items.size())
            return false;
        if(m_items[m_index].isDirectory)
            return false;
        ...
        std::string_view const& outputPath)
    {
        ...
        bool encrypting = false;
        {
            ...
            for(auto const& originalName : m_files) {
                ...
                DATArchiveEncryption::getKeyHashFile(name, headerKeyBase, headerKeyStep, entry.keyBase, entry.keyStep);
            ...
            for(auto const& entry : entries) {
                recordBuf.clear();
                if(!writeEntryRecord(recordBuf, entry) || recordBuf.size() > std::numeric_limits<uint32_t>::max()) {
