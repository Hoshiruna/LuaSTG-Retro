...
namespace core
{
    bool WicImageFactory::createFromMemory(LoggingBuffer& log, const void* const data, const uint32_t size_in_bytes, IImage** const output_image)
    {
        ...
        const auto wic_bitmap_source = wic_format_converter ? static_cast<IWICBitmapSource*>(wic_format_converter.get()) : static_cast<IWICBitmapSource*>(wic_bitmap_frame.get());
