...
namespace utf8
{
    ...
    std::string to_string(std::wstring_view const& str)
    {
        ...
        int const result = WideCharToMultiByte(
            CP_UTF8, 0, str.data(), static_cast<int>(str.length()), buffer.data(), length, nullptr, nullptr);
        if(result != length) {
            assert(false);
            buffer.clear();
        }
    ...
    std::wstring to_wstring(std::string_view const& str)
    {
        ...
        int const length = MultiByteToWideChar(
            CP_UTF8, 0, str.data(), static_cast<int>(str.length()), nullptr, 0);
        if(length <= 0) {
            assert(false);
            return buffer;
        }
        ...
        int const result = MultiByteToWideChar(
            CP_UTF8, 0, str.data(), static_cast<int>(str.length()), buffer.data(), length);
        if(result != length) {
            assert(false);
            buffer.clear();
        }
