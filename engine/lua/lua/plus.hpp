...
namespace lua
{
    struct stack_index_t
    {
        ...
        constexpr stack_index_t(T const index)
    ...
    public:
        ...
        explicit stack_balancer_t(lua_State* const state)
            : L(state), N(lua_gettop(state)) {}
    ...
    struct stack_t
    {
        ...
        explicit stack_t(lua_State* const& state)
            : L(state) {}
        ...
        stack_index_t create_array(size_t const size = 0) const
        {
        ...
        template<>
        inline void set_array_value_zero_base(size_t c_index, std::string_view value)
        {
            lua_pushlstring(L, value.data(), value.size());
            lua_rawseti(L, -2, static_cast<int>(c_index + 1));
        }
        ...
        template<>
        inline void set_array_value(stack_index_t index, std::string_view value)
        {
            lua_pushlstring(L, value.data(), value.size());
            lua_rawseti(L, -2, index.value);
        }
        ...
        inline stack_index_t create_map(size_t reserve = 0u) const
        {
        ...
        [[nodiscard]] bool is_non_or_nil(stack_index_t const index) const
        {
            auto const type = lua_type(L, index.value);
