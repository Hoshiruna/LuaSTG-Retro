    ...
    bool DirectWriteTextLayout::build()
    {
        ...
        if(!win32::check_hresult_as_boolean(
               dwrite->CreateTextLayout(
                   text.c_str(), static_cast<UINT32>(text.length()), m_text_format.get(), m_layout_size.x, m_layout_size.y, m_text_layout.put()),
        ...
        if(!win32::check_hresult_as_boolean(
               d2d1->CreateWicBitmapRenderTarget(
                   bitmap.get(), &render_target_properties, render_target.put()),
        ...
        if(m_stroke_width > 0.00001f) {
            ...
            constexpr D2D1_DRAW_TEXT_OPTIONS options = D2D1_DRAW_TEXT_OPTIONS_NO_SNAP | D2D1_DRAW_TEXT_OPTIONS_ENABLE_COLOR_FONT | D2D1_DRAW_TEXT_OPTIONS_DISABLE_COLOR_BITMAP_SNAPPING;
